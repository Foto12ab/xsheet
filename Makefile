...
flake8:
	flake8 xkits_sheet --count --select=E9,F63,F7,F82 --show-source --statistics
	flake8 xkits_sheet --count --exit-zero --max-complexity=10 --max-line-length=127 --statistics
pytest:
	pytest --cov=xkits_sheet --cov-report=term-missing --cov-report=xml --cov-report=html --cov-config=.coveragerc --cov-fail-under=100
pytest-clean:
	rm -rf .pytest_cache
